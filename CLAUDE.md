...
| 38 | 🟡 Uploadé ASC (traitement Apple) | **Build EAS natif iOS #38** — correctif bouton login (haptics blindé) + base Supabase réactivée + keep-alive cron (GitHub Action 48 h, branche par défaut repo basculée `main`→`master` pour que le cron tourne) + compte démo App Review. Buildé + `eas submit` → uploadé. **Restant** : renseigner compte démo (App Review Information → Sign-In Required), attacher build 38 à la version 1.0, re-soumettre, répondre à Apple. |
| AppStore-refus-2 | ✅ Correctifs livrés (build 38) | **2ᵉ refus Apple (Guideline 2.1a) sur build 37** — « No action occurred when we tapped on Se connecter, the button was unresponsive » (iPad Air M3 / iPadOS 26.5). **Les 2 motifs précédents sont éliminés** (l'app démarre = fix polices OK ; nom OK). **Cause racine n°1 (la plus probable) : la base Supabase `mjthnlvcgavcifmgijxt` était EN PAUSE** (plan gratuit → veille après ~7 j d'inactivité ; statut `INACTIVE`). Backend mort → `signInWithPassword` hang → bouton « unresponsive ». **Réactivée via `restore_project`** (données intactes : 9 users / 7 profils / 7 points — la pause gèle, ne supprime rien). ⚠️ **Re-pausera sans trafic → passer Supabase Pro (~25 $/mois) pour publier** (sinon refus récurrent + app cassée en prod). **Cause racine n°2 (corrigée par sécurité) : `haptics.tap()` appelé AVANT `handleLogin()` dans `login.tsx`, et `lib/haptics.ts` sans try/catch** → sur iPad (pas de Taptic Engine) un échec haptique pouvait bloquer le onPress. Fix : `lib/haptics.ts` blindé (`safe()` try/catch + `.catch`), `onPress` appelle `handleLogin()` d'abord. **Compte de démo pour App Review créé + peuplé dans Supabase** : `demo.apple@lovemap.app` / `LoveMapDemo2026!` (+ ami `camille_demo` accepté + 2 points visibles Paris ; login vérifié 200). À renseigner dans App Store Connect → App Review Information → Sign-In Required. 0 nouvelle erreur tsc (16). |
| 37 | ❌ Refusé (2.1a) puis re-soumis | **Build EAS natif iOS #37 — re-soumis à l'App Store** (correctifs refus build 36). **Cause racine n°2 du « placeholder text in the name » identifiée** : le nom App Store Connect était `LoveMap (e5fa1e)` — le suffixe `(e5fa1e)` (auto-ajouté car « LoveMap » seul est **déjà pris** sur l'App Store) ressemble à du placeholder. **Renommé `LoveMap - Carnet intime`** (unique, propre) dans les **deux** localisations FR + EN(US) — le nom sous l'icône reste « LoveMap » (vient du binaire). Build 37 (anti-freeze polices + AppErrorBoundary) buildé + `eas submit` → binaire uploadé, attaché à la version 1.0, **réponse postée au Resolution Center** (explication des 2 correctifs), **« Soumettre à nouveau » cliqué → état « En attente de vérification »**. Réponse Apple attendue sous ~24-48 h. |
| AppStore-refus-3 | ✅ Corrigé + re-soumis (build 38) le 18/06/2026 | **3ᵉ refus Apple — Guideline 2.3.6 Accurate Metadata** sur build 38 (review 16/06/2026, iPad Air M3 / iPadOS 26.5). ⚠️ **Pas un problème de code** : les 2 refus 2.1a précédents sont éliminés (l'app démarre, login OK). Apple : *« the Age Rating indicates the app includes In-App Controls, but we were unable to find Parental Controls or Age Assurance mechanisms »*. **Cause** : dans le questionnaire **Age Rating** (App Store Connect → Informations sur l'app → Classification par âge), le **« In-App Control » Parental Controls** était déclaré présent, alors que LoveMap n'a **ni contrôle parental ni age assurance** (l'age gate inscription = date de naissance **auto-déclarée** + trigger serveur ≥18, ce qui ne compte ni comme Parental Control ni comme Age Assurance pour Apple). **Résolution (métadonnées seules, AUCUN re-build, build 38 reste attaché — pilotage via Chrome MCP dans ASC le 18/06)** : (1) vérifié que **Parental Controls + Validation de l'âge = Non** étaient déjà enregistrés (la modif de Louis avait bien pris ; le refus datait d'avant) ; (2) corrigé au passage **Publicité = Oui → Non** dans « Capacités » du même questionnaire (l'app n'a aucune pub = inexactitude sous la même règle 2.3.6 ; **UGC laissé à Oui** pour la modération Guideline 1.2 ; Messagerie laissée à Oui) — parcouru les 7 étapes pour confirmer que le **18+ reste calculé** (sexualContentOrNudity=FREQUENT intact) ; (3) l'**« erreur inattendue lors de l'envoi pour vérification »** d'hier était un **bug transitoire ASC** → **« Ajouter pour vérification » puis « Envoyer pour vérification » ont réussi sans erreur** → version **« En attente de vérification »**, build 38, réponse Apple attendue ~24-48 h. **Note ASC** : le questionnaire Age Rating (nouveau système 2025) fait geler les captures d'écran Chrome ; piloté en JS via l'accessibility tree. |
| AppStore-refus-4 | ✅ Données démo + réponse Resolution Center postées (18/06) | **4ᵉ refus Apple — Guideline 2.1 Information Needed** sur build 38 (review 18/06/2026 14:31, iPad Air M3 / iPadOS 26.5). ✅ **Vraie progression** : Apple a lancé l'app ET s'est connecté avec le compte démo (login confirmé `last_sign_in 18/06 12:28`) — les refus 2.1a (freeze) et 2.3.6 (métadonnées) sont derrière. Apple : *« Provide an account with friends and moments to validate so we can verify the safety mechanisms. »* → ils veulent tester la **modération (Guideline 1.2 : signaler/bloquer)** + le **consentement de taguage**. **Cause** : le compte démo `demo_apple` avait un ami (`camille_demo`) mais ses 2 moments étaient créés PAR demo (donc rien à signaler — on ne signale pas ses propres moments) et **aucun taguage en attente** (rien à valider). **Correctif (données Supabase, AUCUN code/build — via MCP Supabase le 18/06)** : ajout de 2 moments **créés par camille → taguant demo** : (1) « Week-end à Montmartre » laissé **`pending`** (is_visible=false) = **moment à valider** par demo dans Amis→Demandes ; (2) « Nuit au canal Saint-Martin » passé **`accepted`** (insert pending puis UPDATE→accepted, le trigger `on_partner_consent` pose is_visible=TRUE) = moment d'ami **visible à signaler/bloquer**. Respecte la règle métier (is_visible via trigger uniquement). **Réponse postée au Resolution Center** (via Chrome MCP, 18/06 20:45) : identifiants démo + pas-à-pas des 3 mécanismes (valider un taguage en attente · signaler un moment · bloquer un utilisateur). ⚠️ **Pour un 2.1 Information Needed, la réponse seule réengage la review** (le bouton « Soumettre à nouveau » est désactivé = pas de re-build/re-soumission attendue). Apple attendu ~24-48 h. **Note** : ASC redemande une connexion (2FA) pour la section Soumissions après un moment → côté Louis si une action « Soumettre à nouveau » devenait nécessaire. **À faire idéalement côté Louis** : copier le pas-à-pas dans les notes persistantes (Vérification de l'app → App Review Information → Notes) pour les reviews futures. Cf. [[lovemap-apple-demo-account]]. |
| AppStore-refus-5 | ✅ Données démo + EULA + réponse + re-soumission (build 38) faites le 19/06 | **5ᵉ refus Apple — Guideline 5 (Legal) + 1.1 (Objectionable Content)** sur build 38 (review 18/06/2026 21:06, iPad Air M3 / iPadOS 26.5). ⚠️ **Refus de CONCEPT, pas technique** : *« the app encourages users to have intimate encounters in public locations »* (répété sur 5 et 1.1). **Audit (18/06)** : la copie in-app ET les métadonnées App Store sont **déjà bien cadrées** (« journal intime privé », « pas une app de rencontre ») — elles n'encouragent rien. **Déclencheur quasi certain = la donnée démo** que le reviewer a vue : les moments étaient épinglés à des **lieux publics emblématiques** (Tour Eiffel, canal Saint-Martin, hôtel de ville, Montmartre). **Correctif (choix Louis : réponse + re-soumission, AUCUN nouveau build)** : (1) **données démo recadrées en contextes PRIVÉS** via MCP Supabase (« Un dimanche à la maison », « Week-end à l'hôtel », « Chez nous au petit matin », « Notre nid ») + coordonnées **hors monuments** (Paris 3e/5e/11e/13e génériques) ; setup de test conservé (1 moment à valider `pending` + 1 moment d'ami visible à signaler) — ⚠️ le reviewer avait **accepté** le taguage pending pendant sa review (preuve qu'il a testé le consentement), restauré en pending. (2) **EULA renforcée** (`docs/terms.html`, hébergée, hors binaire) : clause explicite « carnet privé et rétrospectif, **n'encourage aucun comportement illégal/dangereux/en lieu public** ». **Argument de réponse** : carnet **rétrospectif** (on note un souvenir *après coup*, surtout chez soi), **aucune fonction de découverte/rencontre/lieu public**. **Fait le 19/06 via Chrome MCP** : réponse postée au Resolution Center (21:24) + re-soumission build 38 (« Mettre à jour la vérification » sur la page version → active « Soumettre à nouveau » → cliqué) → version **« En attente de vérification »**. Apple attendu ~24-48 h. ⚠️ **Si re-refus sur le concept** : envisager un nouveau build avec disclaimer in-app visible, ou un appel à l'App Review Board. Cf. [[lovemap-apple-demo-account]]. |
