...
| 37 | 🟡 En attente de vérification Apple | **Build EAS natif iOS #37 — re-soumis à l'App Store** (correctifs refus build 36). **Cause racine n°2 du « placeholder text in the name » identifiée** : le nom App Store Connect était `LoveMap (e5fa1e)` — le suffixe `(e5fa1e)` (auto-ajouté car « LoveMap » seul est **déjà pris** sur l'App Store) ressemble à du placeholder. **Renommé `LoveMap - Carnet intime`** (unique, propre) dans les **deux** localisations FR + EN(US) — le nom sous l'icône reste « LoveMap » (vient du binaire). Build 37 (anti-freeze polices + AppErrorBoundary) buildé + `eas submit` → binaire uploadé, attaché à la version 1.0, **réponse postée au Resolution Center** (explication des 2 correctifs), **« Soumettre à nouveau » cliqué → état « En attente de vérification »**. Réponse Apple attendue sous ~24-48 h. |
