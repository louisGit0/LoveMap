...
| GSD-P3 | 🔄 Code | Phase 3 (Création & Détail, sheets natifs) — **restructuration navigation** : `app/(app)/_layout.tsx` est désormais un `<Stack>`, les 5 écrans tabs sont dans `app/(app)/(tabs)/`, `point/new` + `point/[id]` sont des routes `formSheet` (détent 0.92, poignée, coins natifs). Modal d'aperçu marker supprimée (tap-pin → détail direct). Création/détail refondus « page de carnet » (serif+mono, note d'abord), date détail en segments JJ/MM/AAAA (anti-freeze #2125), dismiss-confirm via `usePreventRemove`. **100% JS, zéro nouvelle dépendance.** En attente validation device. Détail : .planning/phases/03-cr-ation-d-tail-de-point-sheets-natifs/ |
